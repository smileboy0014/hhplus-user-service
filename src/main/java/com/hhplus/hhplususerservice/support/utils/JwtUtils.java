package com.hhplus.hhplususerservice.support.utils;

import com.hhplus.hhplususerservice.domain.common.exception.CustomException;
import com.hhplus.hhplususerservice.infra.redis.RedisRepository;
...
public class JwtUtils {
    ...
    private void changeTokenStatus(String token) {
        redisRepository.deleteKey("waiting:active" + ":" + token);
    }
    ...
    public void validToken(Long userId, String token) {
        ...
        boolean activeToken = redisRepository.setIsMember("waiting:active" + ":" + token, String.valueOf(userId));
        if (!activeToken) {
            throw new CustomException(TOKEN_IS_NOT_FOUND,
